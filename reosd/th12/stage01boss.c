...
20:
	etSpr(0, 3, 1);
	etCnt_rank(0, 1, 1, 1, 2, 10, 10, 10, 10);
...
36:
	etSpr(0, 3, 1);
	etCnt_rank(0, 1, 1, 1, 3, 10, 10, 10, 10);
	etOn(0);
44:
	etSpr(0, 3, 2);
	etOn(0);
52:
	etSpr(0, 3, 1);
	etCnt_rank(0, 1, 1, 3, 4, 10, 10, 10, 10);
	etOn(0);
60:
	etSpr(0, 3, 2);
	etCnt_rank(0, 1, 1, 4, 5, 10, 10, 10, 10);
	etOn(0);
180:
	$A = ($RAND % 3);
...
60:
	etSpr(0, 5, 6);
	etCnt_rank(0, 6, 12, 24, 24, 1, 1, 1, 2);
	etSpd(0, 4.0f, 1.0f);
!ENH
	etAng(0, 0.0f, 0.0f);
!L
...
68:
	etSpr(0, 0, 5);
	etCnt_rank(0, 6, 12, 32, 24, 1, 1, 1, 1);
	etAng(0, 0.0f, 0.0f);
	etSpd(0, 3.0f, 1.0f);
...
92:
	etSpr(0, 5, 6);
	etCnt_rank(0, 6, 12, 24, 32, 1, 1, 1, 2);
!EN
	etAng(0, 0.0f, 0.0f);
!HL
	etAng(0, 0.1308997f, 0.0f);
...
80:
	etMode(0, 0);
	etSpr(0, 7, 2);
	etCnt_rank(0, 2, 3, 5, 5, 16, 16, 16, 16);
	etSpd(0, 5.0f, 1.0f);
...
!NHL
	etMode(0, 2);
	etSpr(0, 5, 6);
	etCnt_rank(0, -1, 16, 24, 48, -1, 1, 2, 3);
...
sub Boss2_at2()
{
	...
	etSpr(0, 7, 10);
	etSpr(1, 0, 10);
	etCnt_rank(0, 24, 36, 48, 48, 1, 1, 2, 2);
	etCnt_rank(1, 28, 28, 28, 32, 1, 1, 1, 2);
!ENH
	etSpd(0, 2.0f, 1.0f);
	etSpd(1, 2.6f, 1.0f);
!L
...
!*
	...
	etEx(0, 0, 0, 2, 2, -999999, -999999.0f, -999999.0f);
60:
	etCnt_rank(0, 4, 8, 12, 16, 2, 2, 2, 2);
	etOn(0);
80:
	etCnt_rank(0, 5, 9, 15, 23, 2, 2, 2, 2);
	etOn(0);
100:
	etCnt_rank(0, 7, 10, 14, 20, 2, 2, 2, 2);
	etOn(0);
220:
	$A = ($RAND % 3);
...
BossCard2_at1_260:
	34;
BossCard2_at1_280:
	etCnt_rank(0, 18, 22, [-1], 34, 1, 1, 1, 2);
	etSpd(0, 4.0f, 1.0f);
	etEx(0, 0, 0, 2, 2, -999999, -999999.0f, -999999.0f);
	%C = 1.570796f;
